...
unsigned int sysctl_sched_game_mode __read_mostly = 0;
	...
	if (clamp_id == UCLAMP_MIN) {
        ...
        if (unlikely(p->cold_launch_boost_end)) {
            if (time_before(jiffies, p->cold_launch_boost_end)) {
                ...
                unsigned int boost_val = max_t(unsigned int, uc_req.value, SCHED_CAPACITY_SCALE);
...
unlock:
	/* Manually release the lock on all return paths */
...
static struct ctl_table sched_core_sysctls[] = {
	...
	{
		.procname   = "sched_game_mode",
		.data       = &sysctl_sched_game_mode,
		.maxlen     = sizeof(unsigned int),
		.mode       = 0644,
...
static void prev_balance(struct rq *rq, struct rq_flags *rf)
{
	 ...
	 */
	for_class_range(class, start_class, &idle_sched_class) {
				 ...
				 unsigned long *max)
{
	...
	if (min) {
		...
		*min = max(irq + cpu_bw_dl(rq), uclamp_rq_get(rq, UCLAMP_MIN));
		...
		if (unlikely(sysctl_sched_game_mode)) {
            /* * Strip away uclamp_min boosts (like touch boosts). 
             * Revert strictly to base hardware/deadline minimums.
             */
	...
	if (max) {
		*max = min(scale, uclamp_rq_get(rq, UCLAMP_MAX));
		...
		if (unlikely(sysctl_sched_game_mode)) {
            unsigned long game_cap = (scale * 80) / 100;
            if (*max > game_cap)
                *max = game_cap;
        }
	}
