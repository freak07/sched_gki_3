...
extern unsigned int sysctl_sched_game_mode;
...
{
	struct sched_entity __maybe_unused *curr = cfs_rq->curr;
	long weight = cfs_rq->sum_weight;
	s64 delta = 0;
...
static inline unsigned int get_effective_rampup_multiplier(struct task_struct *p)
{
	/* 1. Cold Launch: Maximum burst for tasks currently launching */
	if (unlikely(p->cold_launch_boost_end && time_before(jiffies, p->cold_launch_boost_end)))
		return 16;

	/* 2. Device Idle: Disable util_est rampup to save power */
	if (unlikely(sched_get_display_idle()))
		return 0;

	/* 3. Swipes: Boost responsiveness during touch interactions */
	int swipe_state = sched_get_current_swipe();
	if (unlikely(swipe_state != SWIPE_NONE)) {
		/* Give the foreground DRM app a huge multiplier, and background tasks a moderate one */
		if (is_drm_top_app(p)) {
			switch (swipe_state) {
			case SWIPE_STRONG:
		...
		/* Small boost for other system threads processing the swipe */
		return max(2U, p->sched_qos.rampup_multiplier);
	}

	/* 4. Default: Fallback to the task's assigned QoS or the global default */
	return p->sched_qos.rampup_multiplier;
}
				   ...
				   bool task_sleep)
{
	...
	rampup_mult = get_effective_rampup_multiplier(p);

	/*
	 * rampup_multiplier = 0 indicates util_est is disabled.
	 */
	if (sched_feat(UTIL_EST_RAMPUP_ZERO) && !rampup_mult) {
		ewma = 0;
		goto done;
	}
	...
	if (!task_sleep) {
		if (task_util(p) > task_util_dequeued(p) &&
			...
			do_div(delta, 1000);
			ewma = approximate_util_avg(prev_ewma, delta * rampup_mult);
			/*
